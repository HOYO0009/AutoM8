...
| Constrain LLM output at the provider boundary. | Used | Any feature that turns model output into UI-visible application data or executable automation actions. | `server/automation-builder/draftGenerator.ts`, `server/automation-runner/executionPlanner.ts`, `server/automation-runner/adaptiveDesktopExecutor.ts`, `server/automation-builder/draftGenerator.test.ts`, `server/automation-runner/executionPlanner.test.ts` | Request structured output, validate it locally, and return actionable errors or safe fallbacks when the provider cannot satisfy the contract. |
| Centralize provider-boundary mechanics while keeping domain validation local. | Used | Any feature that calls the same provider protocol from multiple domain modules. | `server/llm/openRouterStructuredOutput.ts`, `server/automation-builder/draftGenerator.ts`, `server/automation-runner/executionPlanner.ts`, `server/automation-runner/adaptiveDesktopExecutor.ts` | Shared OpenRouter request, timeout, provider error, assistant-content, and JSON parsing logic lives in one adapter; draft, plan, and action validation stays with the owning module. |
