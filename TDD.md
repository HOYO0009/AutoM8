...
|---|---|---|---|---|---|
| Automation Builder Draft Automation Creation | `documentation/vertical-slices/automation-builder/prompt-to-draft-automation.md` | Unit / contract boundary / component render | `server/automation-builder/draftGenerator.test.ts`, `server/apiErrorResponse.test.ts`, `src/api/autom8Api.test.ts`, `src/automationBuilder/AutomationBuilderPane.test.tsx` | Mocked structured-output provider response and mocked browser `fetch` | Add API integration coverage when clarification sessions or picker-backed answers are added. |
...
| Automation Builder Saved Automation Prompt Edit | `documentation/vertical-slices/automation-builder/edit-saved-automation-with-prompt.md` | Unit / contract boundary / component render | `server/automation-builder/draftGenerator.test.ts`, `server/automation-builder/savedAutomationCandidateStore.test.ts`, `server/automation-runner/automationRunStore.test.ts`, `src/api/autom8Api.test.ts`, `src/automationRunner/SavedAutomationCandidateList.test.tsx` | Mocked structured-output provider response and mocked browser `fetch` | Add browser interaction coverage when an E2E harness is introduced. |
