...
| Automation Builder Saved Automation Prompt Edit | `documentation/vertical-slices/automation-builder/edit-saved-automation-with-prompt.md` | Unit / contract boundary / component render | `server/automation-builder/draftGenerator.test.ts`, `server/automation-builder/savedAutomationStore.test.ts`, `server/automation-runner/automationRunStore.test.ts`, `src/api/autom8Api.test.ts`, `src/automationRunner/SavedAutomationList.test.tsx` | Mocked structured-output provider response and mocked browser `fetch` | Add browser interaction coverage when an E2E harness is introduced. |
